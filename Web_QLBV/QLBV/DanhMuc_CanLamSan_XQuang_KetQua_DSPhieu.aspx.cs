...
namespace QLBV
{
    public partial class DanhMuc_CanLamSan_XQuang_KetQua_DSPhieu : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (id == null)
            {
                Response.Redirect("DanhMuc_CanLamSan_XQuang_KetQua_BenhNhan.aspx");
            }
        }
        ...
        private void loadData()
        {
            ...
            conn.ConnectionString = connectionString;
            conn.Open();
            SqlCommand cmd = new SqlCommand("SELECT      tbl_Phieu.MaPhieu as 'Mã phiếu', tbl_Phieu.LanThu as 'Lần thứ', tbl_Phieu.NgayYeuCau as 'Ngày yêu cầu', tbl_ChiTietXQuang.TenXQuang as 'Chỉ tiêu X-quang', tbl_Phieu.YeuCauChup as 'Yêu cầu X-quang', tbl_Phieu.KQChup as 'Kết quả X-quang', tbl_Phieu.NgayGhiKQ as 'Ngày ghi kết quả', tbl_Phieu.BacSiDieuTri as 'Bác sĩ điều trị', tbl_Phieu.BacSiChuyenKhoa as 'Bác sĩ chuyên khoa' FROM       tbl_Phieu, tbl_ChiTietXQuang , tbl_HoSoBenhAn WHERE     (tbl_HoSoBenhAn.MaBenhNhan = @id) AND (tbl_HoSoBenhAn.MaDieuTri = tbl_Phieu.MaDieuTri) AND (tbl_Phieu.MaChiTieuXQuang IS NOT NULL) AND (tbl_ChiTietXQuang.MaXQuang = tbl_Phieu.MaChiTieuXQuang)", conn);
