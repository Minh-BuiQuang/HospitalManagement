...
namespace QLBV
{
    public partial class DanhMuc_CanLamSan_XQuang_PhanLoai : System.Web.UI.Page
    {
        ...
        protected void LoadList(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("select * from tbl_LoaiXQuang where MaLoaiXQuang = @ma", conn);
            ...
            while (r.Read())
            {
                txt_TenLoai.Text = r["TenLoaiXQuang"].ToString();
            }
            r.Close();
            conn.Close();
        ...
        private void loadGV()
        {
            ...
            conn.ConnectionString = connectionString;
            conn.Open();
            SqlCommand cmd = new SqlCommand("select * from  tbl_LoaiXQuang", conn);
        ...
        private void add()
        {
            ...
            {
                string j = "";
                j = r["TenLoaiXQuang"].ToString();
                if (ten.Trim() == j.Trim())
                {
                    lbl_error.Text = "[ " + ten + " ] đã có trong Danh mục Loại X-Quang";
                    error = 1;
                }
            }
            r.Close();
            if (error == 0)
            {
                SqlCommand cmd1 = new SqlCommand("insert into tbl_LoaiXQuang values(@ma, @ten)", conn);
        ...
        private void del()
        {
            SqlConnection conn = new SqlConnection();
            conn.ConnectionString = connectionString;
            SqlCommand cmd = new SqlCommand("delete from tbl_LoaiXQuang where MaLoaiXQuang = @ma", conn);
        ...
        private void update()
        {
            ...
            if (error == 0)
            {
                SqlCommand cmd1 = new SqlCommand("update tbl_LoaiXQuang set TenLoaiXQuang = @ten where MaLoaiXQuang = @ma", conn);
                cmd1.Parameters.AddWithValue("@ma", ma);
                cmd1.Parameters.AddWithValue("@ten", ten);
                cmd1.ExecuteNonQuery();
        ...
        private string createAutoCode()
        {
            ...
            if (so < 10)
            {
                return "LXQ0" + so.ToString();
            }
            else
                return "LXQ" + so.ToString();
        }
