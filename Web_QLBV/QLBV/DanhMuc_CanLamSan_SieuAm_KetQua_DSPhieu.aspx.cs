...
namespace QLBV
{
    public partial class DanhMuc_CanLamSan_SieuAm_KetQua_DSPhieu : System.Web.UI.Page
    {
        ...
        private void loadData()
        {
            ...
            conn.ConnectionString = connectionString;
            conn.Open();
            SqlCommand cmd = new SqlCommand("SELECT      tbl_Phieu.MaPhieu as 'Mã phiếu', tbl_Phieu.LanThu as 'Lần thứ', tbl_Phieu.NgayYeuCau as 'Ngày yêu cầu', tbl_ChiTietSieuAm.TenSieuAm as 'Chỉ tiêu siêu âm', tbl_Phieu.YeuCauSieuAm as 'Yêu cầu siêu âm', tbl_Phieu.KQSieuAm as 'Kết quả siêu âm', tbl_Phieu.NgayGhiKQ as 'Ngày ghi kết quả', tbl_Phieu.BacSiDieuTri as 'Bác sĩ điều trị', tbl_Phieu.BacSiChuyenKhoa as 'Bác sĩ chuyên khoa' FROM       tbl_Phieu, tbl_ChiTietSieuAm , tbl_HoSoBenhAn WHERE     (tbl_HoSoBenhAn.MaBenhNhan = @id) AND (tbl_HoSoBenhAn.MaDieuTri = tbl_Phieu.MaDieuTri) AND (tbl_Phieu.MaChiTieuSieuAm IS NOT NULL) AND (tbl_ChiTietSieuAm.MaSieuAm = tbl_Phieu.MaChiTieuSieuAm)", conn);
        ...
        {
            string id = Request.QueryString["ID"];
            Response.Redirect("DanhMuc_CanLamSan_SieuAm_KetQua_Phieu.aspx?id=" + id);
        }
        ...
        protected void gv_PatientList_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                colID.NavigateUrl = "DanhMuc_CanLamSan_SieuAm_KetQua_Phieu.aspx?code=" + dataItem[0].ToString().Trim();
